...
<?php
    ...
    $id=isset($_REQUEST['x']) ? $_REQUEST['x'] : $_REQUEST['id'];
    if(!$id){
   ...
   $thirty=$_POST['thirty'];
    ...
    $sql="update contract set customer_sign='$thirty', contract_sign_date_customer='".date("Y-m-d")."'  where id='$id'";
    echo var_dump($sql);
	$result=mysqli_query($con,$sql);
	if(mysqli_affected_rows($con)==1)
	{
    echo "<script>alert('sign submited success');</script>";
    echo ".'<script>window.location.href='print?id=$id';</script>.'";
	}
	else{
...
?>
  <form method="POST">
    <h6>Customer Signature</h6>
      <div class="wrapper">
        <canvas id="signature-pad" class="signature-pad" width=300 height=250></canvas>
